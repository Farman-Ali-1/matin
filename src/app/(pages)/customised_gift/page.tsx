...
export default function Page() {
  ...
  const handleAddToCart = () => {
    const cartData = {
      packaging: selectedPackaging !== null ? packagings[selectedPackaging] : null,
  ...
  return (
    <div className="flex flex-col lg:flex-row px-4 py-6 w-full bg-white gap-6">
      ...
      <div className="w-full lg:w-2/3">
        <div className="mb-6">
          <h1 className="text-3xl font-semibold mb-2">Customise Your Bateel Gift</h1>
          <p className="text-gray-600">
            Create your bespoke gift box from exquisite packaging, filled with your favorite gourmet dates.
          </p>
        </div>
        ...
        {/* Step 1: Packaging */}
        {step === 1 && (
          <div id="boxes" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {packagings.map((item, index) => (
              ...
              >
                <Image src={item.image} width={250} height={250} alt={item.title} className="mx-auto" />
                <h2 className="mt-4 text-lg font-semibold text-center">{item.title}</h2>
        ...
        {/* Step 2: Size */}
        {step === 2 && (
          <div id="sizes" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {sizes.map((size, index) => (
              ...
              >
                <Image src={date1} width={250} height={200} alt={size.label} className="mx-auto" />
        ...
        {step === 3 && (
          <div id="fillings">
            <p className="mb-2 text-gray-600">
              You can select <strong>{sizes[selectedSize!].capacity}</strong> fillings.{' '}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {dateFillings.map((item, index) => (
                ...
                >
                  <Image src={item.image} width={250} height={180} alt={item.title} className="mx-auto" />
      ...
      <div className="w-full lg:w-1/3 bg-gray-50 p-6 rounded-lg shadow-sm">
        ...
        <div className="mb-3">
          ...
          {selectedPackaging !== null && <p>{packagings[selectedPackaging].title}</p>}
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {Object.keys(selectedDates).map((i) => (
              <div key={i} className="flex flex-col items-center text-xs">
                <Image
                  src={dateFillings[parseInt(i)].image}
                  width={40}
                  height={40}
                  alt={dateFillings[parseInt(i)].title}
                  className="rounded"
                />
                <span>{selectedDates[parseInt(i)]}</span>
              </div>
            ))}
          </div>
          <p className="text-sm text-right mt-1 text-gray-500">
            {getTotalSelected()}/{selectedSize !== null ? sizes[selectedSize].capacity : 0}
          </p>
        </div>

        {/* Total and CTA */}
        <div className="mt-4 border-t pt-4">
          <p className="text-lg font-bold">Price per box: ${getTotalPrice().toFixed(2)}</p>
          <button
